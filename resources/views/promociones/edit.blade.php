<x-app-layout title="Editar Promoción - Mama2s Gym">
    <div class="py-12 bg-gray-50">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6">
                <h1 class="text-3xl font-bold text-gray-900">Editar Promoción</h1>
                <p class="text-gray-600 mt-2">Modifica los datos de la promoción</p>
            </div>

            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('promociones.update', $promocion) }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="descripcion" 
                                      id="descripcion" 
                                      rows="3"
                                      class="w-full px-4 py-2 bg-white border border-gray-300 rounded-md text-black focus:ring-orange-500 focus:border-orange-500 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $promocion->descripcion) }}</textarea>
                            @error('descripcion')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="tipo_descuento" 
                                ...
                                <option value="porcentaje" {{ old('tipo_descuento', $promocion->tipo_descuento) === 'porcentaje' ? 'selected' : '' }}>Porcentaje (%)</option>
                                <option value="monto" {{ old('tipo_descuento', $promocion->tipo_descuento) === 'monto' ? 'selected' : '' }}>Monto Fijo ($)</option>
                            </select>
                        ...
                        <div id="monto_descuento_field" style="display: none;">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" 
                                       ...
                                       step="0.01"
                                       min="0"
                                       value="{{ old('monto_descuento', $promocion->monto_descuento) }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="membresia_id" 
                                ...
                                @foreach($membresias as $membresia)
                                    <option value="{{ $membresia->id }}" {{ old('membresia_id', $promocion->membresia_id) == $membresia->id ? 'selected' : '' }}>
